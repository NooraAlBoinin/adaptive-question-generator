...
if not (api_key and api_key.startswith("sk-")):
    st.error("Please provide a valid OpenAI API key in Streamlit Secrets.")
    st.stop()
...
with st.container(border=True):
    ...
    if q and options:
        ...
        if submit:
            ...
            # Feedback (no extra icons)
            if is_correct:
                st.session_state.feedback = {"correct": True, "message": "Correct."}
            else:
                st.session_state.feedback = {
                    "correct": False,
                    "message": f"Incorrect. Correct answer: **{q['correct_answer']}**",
                }
...
with st.container(border=True):
    ...
    if not gm:
        ...
        # Optional: Success by grade/topic/bloom
        by_cfg = (
